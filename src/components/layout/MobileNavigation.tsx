...
import { 
  ...
  Users, GraduationCap, MessageSquare, Trophy, Shield
} from "lucide-react";
...
export const MobileNavigation = () => {
  ...
  return (
    <div className="lg:hidden">
                  ...
                  <div className="p-3 border-t border-border/50">
                    <Link to="/ai-tutors" onClick={() => setIsMenuOpen(false)}>
                      <Button className="w-full font-sans text-sm" size="sm">
                        <Bot className="h-4 w-4 mr-2" /> Start AI Session
                      </Button>
                    </Link>
                  </div>
                </div>
      ...
      <nav aria-label="Primary" className="fixed bottom-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-lg border-t border-border/50 safe-bottom">
          ...
          {bottomNavItems.map((item) => {
            ...
            const content = (
              <>
                ...
                <item.icon className={cn("h-[22px] w-[22px] transition-transform", active && "scale-110")} strokeWidth={active ? 2.4 : 2} />
                <span className={cn("text-[10px] leading-none font-sans", active && "font-semibold")}>{item.label}</span>
              </>
            );

            if (item.isMenu) {
              return <button key="menu" onClick={() => setIsMenuOpen(true)} aria-label="Open menu" className={baseClass}>{content}</button>;
            }
            return <Link key={item.href} to={item.href} aria-current={active ? "page" : undefined} className={baseClass}>{content}</Link>;
          })}
        </div>
      </nav>
    </div>
