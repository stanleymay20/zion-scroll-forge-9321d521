...
-- Scope dangling-session integrity to sessions created during the simulation run window,
-- and make session creation idempotent (ON CONFLICT) so same-day re-runs don't error.
...
BEGIN
  ...
  FOR v_section IN SELECT s FROM unnest(v_sec_ids) s LOOP
    ...
    BEGIN
      -- Idempotent same-day session creation
      INSERT INTO public.section_attendance_sessions(section_id, session_date, topic, created_by)
        ...
        ON CONFLICT (section_id, session_date) DO UPDATE SET topic = EXCLUDED.topic
  ...
  PERFORM public._csim_stage(v_run_id, 3, 'attendance', v_count_att_total, v_stage_start, 0,
    jsonb_build_object('attendance_records', v_count_att_total, 'last_error', v_last_err));
