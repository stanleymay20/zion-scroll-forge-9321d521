...
BEGIN
  ...
  -- Prerequisite check: a course requirement is satisfied when the student
  -- either reached 100% course progress or holds a finalized passing grade.
        ...
        AND EXISTS (
          ...
          WHERE NOT (
            ...
            OR EXISTS (
              ...
              WHERE gr.student_user_id = _user
                AND gr.course_id::text = req.req_id
                AND COALESCE(gr.is_final,false) = true
                AND COALESCE(gr.grade_status,'') IN ('provisional','final','posted')
