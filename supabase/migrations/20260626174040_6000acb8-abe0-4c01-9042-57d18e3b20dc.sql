...
BEGIN
  ...
  INSERT INTO public.cohort_simulation_runs(label, requested_size, started_by, status)
  ...
  -- Stage 1: matriculation -- profile + degree enrollment
  v_stage_start := clock_timestamp();
  ...
  -- Stage 2: registration via REAL RPC, impersonated per student
  v_stage_start := clock_timestamp();
  FOREACH v_student IN ARRAY p_student_ids LOOP
  ...
  -- Stage 6: degree audit + graduation
  v_stage_start := clock_timestamp();
  FOREACH v_student IN ARRAY p_student_ids LOOP
    BEGIN
