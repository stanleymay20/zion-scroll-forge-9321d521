...
-- 1) Drop legacy check_graduation_eligibility(uuid) — canonical is (_user_id, _program_id)
DROP FUNCTION IF EXISTS public.check_graduation_eligibility(uuid);

-- 2) Fix evaluate_graduation_candidate to write canonical graduation_candidates.status values
--    Allowed: pending, approved, denied, graduated, deferred
...
BEGIN
  ...
  -- Canonical statuses per graduation_candidates_status_check:
  --   approved  = ready to confer (was 'eligible')
  --   pending   = under review     (was 'pending_review')
  --   denied    = requirements not met (was 'blocked')
...
-- 3) Update simulate_cohort: pass v_term to evaluate_academic_standing (canonical 2-arg signature)
...
BEGIN
  ...
  FOREACH v_student IN ARRAY p_student_ids LOOP
    BEGIN
      PERFORM public.compute_student_gpa(v_student, v_term);
      v_count_gpa := v_count_gpa + 1;
      PERFORM public.evaluate_academic_standing(v_student, v_term);
  ...
  SELECT count(*) INTO v_audit_count
    ...
    WHERE ara.student_id = ANY(p_student_ids);
  ...
  ELSIF v_fail_rate > 25 THEN
    v_verdict := 'needs_stabilization';
  ELSE
    v_verdict := 'ready_for_release';
  END IF;
...
-- 4) Data hygiene: clean genuinely orphaned attendance sessions older than 30 days
DELETE FROM public.section_attendance_sessions s
WHERE s.session_date < (current_date - INTERVAL '30 days')
  AND NOT EXISTS (
    SELECT 1 FROM public.section_attendance_records r WHERE r.session_id = s.id
  );
